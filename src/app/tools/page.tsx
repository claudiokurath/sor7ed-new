...
export const metadata = { title: 'Tools — SOR7ED', description: 'Practical tools for neurodivergent brains.' };
export const revalidate = 60;

export default async function ToolsPage() {
  const tools = await getTools();
  const live = tools.filter((t: any) => t.status === 'live');
  const soon = tools.filter((t: any) => t.status !== 'live');

  return (
    <div style={{ minHeight: '100vh', background: '#08080A' }}>
      <SEOJsonLd title="Tools" description="Practical tools for neurodivergent brains." slug="tools" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1px', background: 'rgba(255,255,255,0.04)', marginBottom: '4rem' }}>
              {live.map((tool: any) => (
                <Link key={tool.slug} href={'/tools/' + tool.slug}
                  style={{ background: '#08080A', padding: '2rem', display: 'block', textDecoration: 'none', position: 'relative', overflow: 'hidden', transition: 'background 0.2s' }}
